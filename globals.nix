...
let
  ...
  libFunctions = import ./lib/functions.nix { inherit lib; };

  # Create hostname-bound versions of functions that need it
  isCurrentHost = libFunctions.isCurrentHost hostname;
  syncthingMachinesWithFolder = libFunctions.syncthingMachinesWithFolder hostname;
...
in
{
  ...
  fn = {
    ...
    # Functions that need the current hostname
    inherit isCurrentHost syncthingMachinesWithFolder;

    # Functions that need additional parameters bound with hostname
    generateSyncthingFolders =
      machine: machines: folders:
      libFunctions.generateSyncthingFolders machine machines folders hostname;

    generateSyncthingDevices =
      machines:
      libFunctions.generateSyncthingDevices machines hostname;
  };
}
